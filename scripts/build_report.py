...
""")
    ...
    # Post-process LaTeX to fix escape issues and ensure consistent layout
    content = Path(tex_path).read_text(encoding="utf-8")
    content = content.replace("\\\\textwidth", "\\textwidth")
    content = content.replace("resizebox{\t", "resizebox{\\textwidth")
    content = content.replace("resizebox{extwidth", "resizebox{\\textwidth")
    content = content.replace("\\\\includegraphics[width=0.95\\textwidth]", "\\includegraphics[width=0.95\\textwidth]")
    content = content.replace("\\\\includegraphics[width=0.8\\textwidth]", "\\includegraphics[width=0.8\\textwidth]")

    # Ensure dataset link block is sloppy to avoid margin overflow
    if "Three public datasets were used" in content and "\\sloppy" not in content:
        content = content.replace(
            "Three public datasets were used to estimate arrival profiles and product mix:\n",
            "Three public datasets were used to estimate arrival profiles and product mix:\n\\sloppy\n",
        )
        content = content.replace("\\end{itemize}\n\nStory of instance collection and generation:", "\\end{itemize}\n\\fussy\n\nStory of instance collection and generation:")

    # Replace plots section with improved visuals and bullet observations
    start = content.find("\\section{Plots by Dataset}")
    end = content.find("\\section{Top-10 Scenarios (Maven)}")
    if start != -1 and end != -1:
        new_plots = r"""\section{Plots by Dataset}
\subsection*{Maven}
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{maven_coffee_shop_sales_distributions.png}
...
\end{itemize}
"""
        content = content[:start] + new_plots + content[end:]

    # Ensure Conclusions section exists
    if "\\section{Conclusions}" not in content:
        content = content.replace(
            "\\section{Recommendations}\n",
            "\\section{Conclusions}\nThe simulation confirms that preparation is the dominant bottleneck under peak-like demand, while ordering and pickup contribute marginally to total delay. Maven-derived scenarios show high utilization and abandonment, indicating that staffing or process improvements should prioritize the preparation stage. Kaggle and Hugging Face scenarios remain low-load baselines and should be scaled if used to emulate peak conditions.\n\n\\section{Recommendations}\n",
        )
